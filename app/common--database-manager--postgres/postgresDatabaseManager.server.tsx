...
declare global {
    ...
    // eslint-disable-next-line no-var
    var _postgresDatabaseManagers: Map<Uuid | null, PostgresDatabaseManager>;
}
...
export class PostgresDatabaseManager {
    ...
    ): Promise<QueryResult<unknown> | Error> {
        try {
...
export async function getPostgresDatabaseManager(
    id?: Uuid | null,
): Promise<PostgresDatabaseManager | Error> {
    if (id === undefined) {
        id = null;
    }
    ...
    if (!global._postgresDatabaseManagers.has(id)) {
        if (global._postgresDatabaseCredentialsResolver == null) {
            return new Error('edc6b564-0c19-43d6-ad07-74601e74f4c2');
        }

        const postgresDatabaseCredentials =
            global._postgresDatabaseCredentialsResolver(id);

        if (postgresDatabaseCredentials instanceof Error) {
            return postgresDatabaseCredentials;
        }

        const databaseConnectionPool = await getNewDatabaseConnectionPool(
...
export async function getNewDatabaseConnectionPool(
    credentials: PostgresDatabaseCredentials,
): Promise<pg.Pool | Error> {
